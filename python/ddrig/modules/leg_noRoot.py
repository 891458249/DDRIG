...
class LegNoRoot(ModuleCore):
    ...
    The body of this class is intentionally a verbatim copy of leg's
    ``Leg`` -- the IK / mid_lock / ribbon / knee_jDef chain depends on
    a real LegRoot guide + ``_legRoot_jDef`` deform joint, and previous
    attempts to remove or substitute it (alias / build-time anchor)
    silently broke the rig.  leg_noRoot keeps the proven build path
    100% intact and instead tames LegRoot at the **guide** stage
    (see :class:`Guides` below): the guide is created, but
...
class Guides(GuidesCore):
    """Guide-stage variant of leg's ``Guides``.

    The full chain (LegRoot, Hip, Knee, ...) is still drawn so the
    build pipeline finds a valid ``inits[0] = LegRoot``.  LegRoot is
    then snapped onto Hip, point-constrained to Hip, hidden, marked
    as a template (un-selectable), and rotate/scale-locked so the
    user never touches it directly."""
    ...
    def draw_joints(self):
        ...
        # rotate/scale channels.  Translate is intentionally left
        # unlocked because the constraint drives it; locking would error.
        try:
